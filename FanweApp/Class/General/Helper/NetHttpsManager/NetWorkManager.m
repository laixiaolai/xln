...
#define Idfv [[UIDevice currentDevice].identifierForVendor UUIDString]//设备uuid
...
+ (NSString *)sortedDictionary:(NSDictionary *)dict{
    ...
    for (NSString *categoryId in afterSortKeyArray) {
        [contentString appendFormat:@"%@=%@&", categoryId, [dict objectForKey:categoryId]];
    }
    NSString *jContentString = [contentString substringToIndex:contentString.length-1];
    NSString *privkey=[AppDelegate sharedAppDelegate].privateKey;
    
    NSString *md5Stringasc;
    NSString *uMd5string;
    if(privkey.length>0){
        //用私钥加密ascll排序后的字符串
        md5Stringasc = [NSString stringWithFormat:@"%@&sigkey=%@",jContentString,privkey];
        uMd5string=[FWMD5UTils getmd5WithString:md5Stringasc];
    }
    return uMd5string;
}
...
+ (NSMutableDictionary *)getLocalParm:(NSMutableDictionary *)parmDict url:(NSString *)urlStr
{
    ...
    NSString *channelId = [infoDict objectForKey:@"channelID"];
    ...
    [parmDict setObject:channelId forKey:@"qudao"];
    ...
    NSString *sigValue= [NetWorkManager sortedDictionary:parmDict];
    //[parmDict setValue:sigValue forKey:@"sig"];
    ...
    if ([IsNeedAES isEqualToString:@"YES"])
    {
        ...
        if (data)
        {
            ...
            if (![FWUtils isBlankString:parmStr])
            {
                ...
                [parmDict setObject:Idfv forKey:@"device_id"];
