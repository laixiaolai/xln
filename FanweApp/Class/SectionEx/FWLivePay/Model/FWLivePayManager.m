    ...
    if (isHost == YES)//是主播
    {
        if (bankerView.hidden)
        {
            if (auctionView.hidden)
            {
                ...
                rect.origin.y = CGRectGetMaxY(self.subLiving.topView.frame)+kStatusBarHeight;
                self.hostLeftPView.frame = rect;
            }
            else
            {
                CGRect rect = self.hostLeftPView.frame;
                rect.origin.y = CGRectGetMaxY(self.subLiving.topView.frame)+auctionView.height+8+kStatusBarHeight;
                self.hostLeftPView.frame = rect;
            }
        }
        else
        {
            ...
            {
                CGRect rect = self.hostLeftPView.frame;
                rect.origin.y = CGRectGetMaxY(self.subLiving.topView.frame)+bankerView.height+8+kStatusBarHeight;
                self.hostLeftPView.frame = rect;
            }
            else
            {
                CGRect rect = self.hostLeftPView.frame;
                rect.origin.y = CGRectGetMaxY(self.subLiving.topView.frame)+bankerView.height+auctionView.height+8+8+kStatusBarHeight;
                self.hostLeftPView.frame = rect;
            }
        }
    }else//是观众
