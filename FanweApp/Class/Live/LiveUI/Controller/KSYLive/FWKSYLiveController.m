...
- (void)viewDidLoad
{
    ...
    self.fwBeautyV = [[FWFUBeautyView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
    [self.liveServiceController.liveUIViewController.liveView addSubview:self.fwBeautyV];
}
...
- (void)selectToolsItemWith:(ToolsView *)toolsView selectIndex:(NSInteger)index isSelected:(BOOL)isSelected
{
    ...
    { // 美颜
        //隐藏金山美颜
//        _beautyView.hidden = YES;
//        self.fwBeautyV.hidden=YES;
        self.fwBeautyV.hidden=NO;
    }
    else if (index == 2)
    { // 麦克风
        if (isSelected == YES)
