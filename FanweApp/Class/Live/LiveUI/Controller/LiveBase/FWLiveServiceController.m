...
@property(nonatomic,strong)NSMutableArray <UserModel*>* driverArr;
@property(nonatomic,assign)BOOL isDriverAnimationPlaying;//是否在播放座驾动画
@end
...
- (void)initFWVariables
{
    ...
    self.driverArr=[[NSMutableArray alloc]init];
}
...
-(void)updateZz{
    ...
    NSMutableDictionary *mDict = [NSMutableDictionary dictionary];
    [mDict setObject:@"video" forKey:@"ctl"];
    [mDict setObject:@"live_room_limit_name" forKey:@"act"];
    FWWeakify(self)
    ...
    [self.httpsManager POSTWithParameters:mDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        if ([responseJson toInt:@"status"] == 1)
        {
            self.fanweApp.appModel.limit_name=[responseJson toString:@"limit_name"];
        }
...
#pragma mark 收到自定义的Group消息 腾讯IM消息
- (void)onIMHandler:(FWIMMsgHandler *)receiver recvCustomGroup:(id<AVIMMsgAble>)msg
{
    ...
    switch (customMessageModel.type)
    {
        ...
        case MSG_VIEWER_JOIN: //5：观众进入直播间消息
        {
            ...
            userModel.is_vip=[NSString stringWithFormat:@"%d",customMessageModel.sender.is_vip];
            userModel.driver_code=customMessageModel.driver_code;
            NSLog(@"userModel.driver_code:%@",userModel.driver_code);
            userModel.driver_name=customMessageModel.driver_name;
...
- (void)looperWork
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_isDriverAnimationPlaying==NO && self.driverArr.count>0 && [GlobalVariables sharedInstance].appModel.open_driver  && _currentBigGiftState==0) {
            _isDriverAnimationPlaying=YES;
            
            [self playDriverAnimation];
        }
        else if (_currentBigGiftState == 0 && [_gifAnimateArray count] && _isDriverAnimationPlaying==NO )
        {
...
#pragma mark HaidiyoulingViewController代理（HaidiyoulingViewControllerDelegate）
- (void)haidiyoulingAnimationFinished
{
    [self removePlayAnimate:kHaidiyoulingTag];
}

#pragma mark HuangguanViewController代理（HuangguanViewControllerDelegate）
- (void)huangguanAnimationFinished
{
    [self removePlayAnimate:kHuangguanTag];
}

#pragma mark FalaliViewController代理（FalaliViewControllerDelegate）
- (void)falaliAnimationFinished
{
    [self removePlayAnimate:kFalaliTag];
}

#pragma mark KongmingViewController代理（KongmingViewControllerDelegate）
- (void)kongmingAnimationFinished
{
    [self removePlayAnimate:kKongmingTag];
}

#pragma mark HuojianViewController代理（HuojianViewControllerDelegate）
- (void)huojianAnimationFinished
{
    [self removePlayAnimate:kHuojianTag];
}
#pragma mark NisangViewController代理（NisangViewControllerDelegate）
- (void)nisangAnimationFinished
{
    [self removePlayAnimate:kNisangTag];
}

#pragma mark ChengbaoViewController代理（ChengbaoViewControllerDelegate）
- (void)chengbaoAnimationFinished
{
    [self removePlayAnimate:kChengbaoTag];
}

#pragma mark QianlichuanqingViewController代理（QianlichuanqingViewControllerDelegate）
- (void)qianlichuanqingAnimationFinished
{
    [self removePlayAnimate:kQianlichuanqingTag];
}

#pragma mark YoulunViewController代理（YoulunViewControllerDelegate）
- (void)youlunAnimationFinished
{
    [self removePlayAnimate:kYoulunTag];
}

#pragma mark AixinmeiguiViewController代理（AixinmeiguiViewControllerDelegate）
- (void)aixinmeiguiAnimationFinished
{
    [self removePlayAnimate:kAixinmeiguiTag];
}

#pragma mark FeixuexiaozhenViewController代理（FeixuexiaozhenViewControllerDelegate）
- (void)feixuexiaozhenAnimationFinished
{
    [self removePlayAnimate:kFeixuexiaozhenTag];
}

#pragma mark ZhishengjiViewController代理（ZhishengjiViewControllerDelegate）
- (void)zhishengjiAnimationFinished
{
    [self removePlayAnimate:kZhishengjiTag];
}

#pragma mark ZhushishunliViewController代理（ZhushishunliViewControllerDelegate）
- (void)zhushishunliAnimationFinished
{
    [self removePlayAnimate:kZhushishunliTag];
}

#pragma mark ZuanjieViewController代理（ZuanjieViewControllerDelegate）
- (void)zuanjieAnimationFinished
{
    [self removePlayAnimate:kZuanjieTag];
}

#pragma mark TianshiViewController代理（TianshiViewControllerDelegate）
- (void)tianshiAnimationFinished
{
    [self removePlayAnimate:kTianshiTag];
}

#pragma mark QiubiteViewController代理（QiubiteViewControllerDelegate）
- (void)qiubiteAnimationFinished
{
    [self removePlayAnimate:kQiubiteTag];
}

#pragma mark XingqiuViewController代理（XingqiuViewControllerDelegate）
- (void)xingqiuAnimationFinished
{
    [self removePlayAnimate:kXingqiuTag];
}

#pragma mark YingmuhuadaoViewController代理（YingmuhuadaoViewControllerDelegate）
- (void)yingmuhuadaoAnimationFinished
{
    [self removePlayAnimate:kYingmuhuadaoTag];
}

#pragma mark JitaViewController代理（JitaViewControllerDelegate）
- (void)jitaAnimationFinished
{
    [self removePlayAnimate:kJitaTag];
}

#pragma mark JiaziguViewController代理（JiaziguViewControllerDelegate）
- (void)jiaziguAnimationFinished
{
    [self removePlayAnimate:kJiaziguTag];
}

#pragma mark JiebaochaopaoViewController代理（JiebaochaopaoViewControllerDelegate）
- (void)jiebaochaopaoAnimationFinished
{
    [self removePlayAnimate:kJiebaochaopaoTag];
}
...
{
    if ([customMessageModel.anim_type isEqualToString:kHaidiyoulingTypeStr])
    {
        HaidiyoulingViewController *tmpController = [[HaidiyoulingViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kHaidiyoulingTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kHuangguanTypeStr])
    {
        HuangguanViewController *tmpController = [[HuangguanViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kHuangguanTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kFalaliTypeStr])
    {
        FalaliViewController *tmpController = [[FalaliViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kFalaliTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kKongmingTypeStr])
    {
        KongmingViewController *tmpController = [[KongmingViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kKongmingTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kHuojianTypeStr])
    {
        HuojianViewController *tmpController = [[HuojianViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kHuojianTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kNisangTypeStr])
    {
        NisangViewController *tmpController = [[NisangViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kNisangTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kChengbaoTypeStr])
    {
        ChengbaoViewController *tmpController = [[ChengbaoViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kChengbaoTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kQianlichuanqingTypeStr])
    {
        QianlichuanqingViewController *tmpController = [[QianlichuanqingViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kQianlichuanqingTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kYoulunTypeStr])
    {
        YoulunViewController *tmpController = [[YoulunViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kYoulunTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kAixinmeiguiTypeStr])
    {
        AixinmeiguiViewController *tmpController = [[AixinmeiguiViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kAixinmeiguiTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kFeixuexiaozhenTypeStr])
    {
        FeixuexiaozhenViewController *tmpController = [[FeixuexiaozhenViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kFeixuexiaozhenTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kZhishengjiTypeStr])
    {
        ZhishengjiViewController *tmpController = [[ZhishengjiViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kZhishengjiTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kZhushishunliTypeStr])
    {
        ZhushishunliViewController *tmpController = [[ZhushishunliViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kZhushishunliTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kZuanjieTypeStr])
    {
        ZuanjieViewController *tmpController = [[ZuanjieViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kZuanjieTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kTianshiTypeStr])
    {
        TianshiViewController *tmpController = [[TianshiViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kTianshiTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kQiubiteTypeStr])
    {
        QiubiteViewController *tmpController = [[QiubiteViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kQiubiteTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kXingqiuTypeStr])
    {
        XingqiuViewController *tmpController = [[XingqiuViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kXingqiuTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kYingmuhuadaoTypeStr])
    {
        YingmuhuadaoViewController *tmpController = [[YingmuhuadaoViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kYingmuhuadaoTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kJitaTypeStr])
    {
        JitaViewController *tmpController = [[JitaViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kJitaTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kJiaziguTypeStr])
    {
        JiaziguViewController *tmpController = [[JiaziguViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kJiaziguTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
    else if ([customMessageModel.anim_type isEqualToString:kJiebaochaopaoTypeStr])
    {
        JiebaochaopaoViewController *tmpController = [[JiebaochaopaoViewController alloc]init];
        tmpController.senderNameStr = customMessageModel.top_title;
        tmpController.delegate = self;
        [self.view addSubview:tmpController.view];
        tmpController.view.tag = kJiebaochaopaoTag;
        tmpController.view.frame = self.view.bounds;
        [self.view sendSubviewToBack:tmpController.view];
    }
}
...
{
    BOOL isVip = [userModel.is_vip isEqualToString:@"1"];
    if ([userModel.user_level integerValue] >= self.fanweApp.appModel.jr_user_level||isVip)
    {
    ...
    if (userModel.driver_code.length>0) {
        [self.driverArr addObject:userModel];
    }
}
...
-(void)playDriverAnimation{
    UserModel* userModel = [self.driverArr firstObject];
    [self.driverArr removeObjectAtIndex:0];
    YTSeeAnimationView* seeV=[[YTSeeAnimationView alloc]init];
    [seeV shouldShowCloseBtn:NO];
    seeV.completeBlock = ^{
        self.isDriverAnimationPlaying=NO;
    };
    [seeV showAnimation:userModel.driver_code text:[NSString stringWithFormat:@"%@乘坐【%@】来了",userModel.nick_name,userModel.driver_name]];
    
    [self.view addSubview:seeV];
    seeV.frame = self.view.bounds;
    [self.view sendSubviewToBack:seeV];
}
#pragma mark - ----------------------- 红包 -----------------------
#pragma mark 展示红包
...
-(void)liveFreeClick{
    self.liveUIViewController.livePay.hostLeftPView.hidden=YES;
    self.liveUIViewController.livePay.audienceLeftPView.hidden=YES;
    //self.liveUIViewController.livePay.realPersonPromptView.hidden=YES;
    //self.liveUIViewController.livePay.hostLeftMoneyView.hidden=YES;
    [self liveFree];
}
#pragma mark 点击免费直播
-(void)liveFree{
    NSMutableDictionary *parameterDic = [NSMutableDictionary new];
    [parameterDic setValue:@"live" forKey:@"ctl"];
    [parameterDic setValue:@"live_free" forKey:@"act"];
    [parameterDic setValue:_roomIDStr forKey:@"room_id"];
    [self.httpsManager POSTWithParameters:parameterDic SuccessBlock:^(NSDictionary *responseJson){
        if ([responseJson toInt:@"status"] == 1) {
            [FanweMessage alertTWMessage:@"已切换为免费模式"];
            self.liveUIViewController.livePay.hostLeftPView.hidden=YES;
            self.liveUIViewController.livePay.audienceLeftPView.hidden=YES;
