...
//    [parmDict setObject:@"live_creater_comeback_msg" forKey:@"act"];
...
- (void)enterChatGroupSucc:(CurrentLiveInfo *)liveInfo
{
    ...
    // 发送加入直播间消息
    if (!_isHost && (liveInfo.join_room_prompt == 1 || [[IMAPlatform sharedInstance].host getUserRank] >= self.fanweApp.appModel.jr_user_level || ([GlobalVariables sharedInstance].driver_code && [GlobalVariables sharedInstance].driver_code.length > 0)))
    {
        SendCustomMsgModel *sendCustomMsgModel = [[SendCustomMsgModel alloc] init];
        sendCustomMsgModel.msgType = MSG_VIEWER_JOIN;
        sendCustomMsgModel.chatGroupID = [self.liveItem liveIMChatRoomId];
        sendCustomMsgModel.driver_code=[GlobalVariables sharedInstance].driver_code;
        sendCustomMsgModel.driver_name=[GlobalVariables sharedInstance].driver_name;
        [_iMMsgHandler sendCustomGroupMsg:sendCustomMsgModel succ:nil fail:nil];
    }
}
...
//                NSMutableDictionary *parmDict = [NSMutableDictionary dictionary];
//                [parmDict setObject:@"live" forKey:@"ctl"];
//                [parmDict setObject:@"live_creater_leave_msg" forKey:@"act"];
//                [parmDict setObject:[_liveItem liveIMChatRoomId] forKey:@"room_id"];
//
//                [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
//                 {
