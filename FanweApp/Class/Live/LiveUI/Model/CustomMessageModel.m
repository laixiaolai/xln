...
//判断是否有脏字
- (BOOL)hasLimitString:(NSString *)limitString andConString:(NSString *)conString{
    NSArray *limitStringArray = [limitString componentsSeparatedByString:@","];
    
    for (NSString *str in limitStringArray) {
        if ([conString containsString:str])
        {
            return YES;
        }
    }
...
- (void)prepareForRender
{
    ...
    NSString *limitString=fanweApp.appModel.limit_name;
    ...
    else if (type == MSG_VIEWER_JOIN)
    {
        //是否vip
        BOOL isVip;
        if ([self isHostLive:self.sender.user_id]) {
            ...
            MJRefreshLog(@"我是vip嘛？%d,%@",isVip,self.text);
        }else{
            isVip = self.sender.is_vip;
            MJRefreshLog(@"其他观众是vip嘛？%d ,%@,谁发的:%@",isVip,self.text,self.nick_name);
        }
//        BOOL isVip = [[GlobalVariables sharedInstance].appModel.open_vip isEqualToString:@"1"] && [[GlobalVariables sharedInstance].is_vip isEqualToString:@"1"];
    ...
    {
        //如果nameStr或contentStr有脏字，直接返回
        if([self hasLimitString:limitString andConString:nameStr]){
            messageStr = @" ";
        }
        else if([self hasLimitString:limitString andConString:contentStr]){
            messageStr = @" ";
        }
        else{
