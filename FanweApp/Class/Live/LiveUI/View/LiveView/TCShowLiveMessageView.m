...
@interface TCShowLiveMsgTableViewCell()
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier])
    {
        ...
        [_msgBack addSubview:_vipImgView];
...
- (void)config:(CustomMessageModel *)item block:(FWVoidBlock)block
{
    ...
    //是否vip
    BOOL isVip;
    if ([self isHostLive:item.sender.user_id]) {
        isVip = [[GlobalVariables sharedInstance].appModel.open_vip isEqualToString:@"1"] && [[GlobalVariables sharedInstance].is_vip isEqualToString:@"1"];
        MJRefreshLog(@"我是vip嘛？%d,%@",isVip,item.text);
    }else{
        isVip = item.sender.is_vip;
        MJRefreshLog(@"其他观众是vip嘛？%d ,%@,谁发的:%@",isVip,item.text,item.nick_name);
    }
    self.isVip=isVip;
    ...
    else
    {
        ...
        if (customMessageModel.sender.user_level)
        {
            ...
            if(isVip){
                ...
                _vipImgView.frame=CGRectMake(0, 4, 13, 13);
...
//            MJRefreshLog(@"我是vip嘛？%d,%@",isVip,item.text);
//        }else{
//            isVip = item.user.is_vip;
//            MJRefreshLog(@"其他观众是vip嘛？%d ,%@",isVip,item.text);
//        }
        if (customMessageModel.sender.user_level >= fanweApp.appModel.jr_user_level||isVip)
        {
...
- (void)relayoutFrameOfSubViews
{
    ...
    _msgBack.frame = rect;
    _msgBack.frame = CGRectMake(CGRectGetMinX(_msgBack.frame), (self.frame.size.height-CGRectGetHeight(_msgBack.frame))/2, COMMENT_TABLEVIEW_WIDTH, CGRectGetHeight(_msgBack.frame));
    ...
    if (_rankImgView.isHidden == NO)
    {
        ...
        //是否vip
//        BOOL isVip = [[GlobalVariables sharedInstance].appModel.open_vip isEqualToString:@"1"] && [[GlobalVariables sharedInstance].is_vip isEqualToString:@"1"];
        if(self.isVip){
            _rankImgView.frame = CGRectMake(_vipImgView.frame.size.width, rankImgViewY, 28, 13);
        }
        else
        {
