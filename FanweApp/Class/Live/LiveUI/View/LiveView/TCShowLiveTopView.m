...
- (void)addOwnViews
{
    ...
    _liveFollow.backgroundColor = kAppMainColor;
...
- (void)addOwnViewsWith:(id<FWShowLiveRoomAble>)room
{
    ...
    //直播间广告
    if(fanweApp.appModel.room_ad_a_name.length>0&&fanweApp.appModel.room_ad_a_url.length>0){
        _adLabel1 = [[UILabel alloc]init];
        _adLabel1.textColor = kWhiteColor;
        _adLabel1.font = [UIFont systemFontOfSize:10.0];
        _adLabel1.text = fanweApp.appModel.room_ad_a_name;
        _adLabel1.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.2f];
        _adLabel1.textAlignment=NSTextAlignmentCenter;
        _adLabel1.layer.cornerRadius=11;
        
        [[_adLabel1 layer] setMasksToBounds:YES];
        
        _adLabel1.userInteractionEnabled = YES;
        UITapGestureRecognizer *tapGesture =
        [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(openURLa:)];
        [_adLabel1 addGestureRecognizer:tapGesture];
        [self addSubview:_adLabel1];
        if(fanweApp.appModel.room_ad_b_name.length>0&&fanweApp.appModel.room_ad_b_url.length>0){
            _adLabel2 = [[UILabel alloc]init];
            _adLabel2.textColor = kWhiteColor;
            _adLabel2.font = [UIFont systemFontOfSize:10.0];
            _adLabel2.text = fanweApp.appModel.room_ad_b_name;
            _adLabel2.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.2f];
            _adLabel2.textAlignment=NSTextAlignmentCenter;
            _adLabel2.layer.cornerRadius=11;
            
            [[_adLabel2 layer] setMasksToBounds:YES];
            
            _adLabel2.userInteractionEnabled = YES;
            UITapGestureRecognizer *tapGesture =
            [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(openURLb:)];
            [_adLabel2 addGestureRecognizer:tapGesture];
            [self addSubview:_adLabel2];
        }
    }
    else if(fanweApp.appModel.room_ad_b_name.length>0&&fanweApp.appModel.room_ad_b_url.length>0){
        _adLabel1 = [[UILabel alloc]init];
        _adLabel1.textColor = kWhiteColor;
        _adLabel1.font = [UIFont systemFontOfSize:10.0];
        _adLabel1.text = fanweApp.appModel.room_ad_b_name;
        _adLabel1.backgroundColor = [[UIColor blackColor]colorWithAlphaComponent:0.2f];
        _adLabel1.textAlignment=NSTextAlignmentCenter;
        _adLabel1.layer.cornerRadius=11;
...
//直播间广告a跳转事件
-(void)openURLa:(UITapGestureRecognizer *)gesture{
    GlobalVariables *fanweApp = [GlobalVariables sharedInstance];
    NSString *url = fanweApp.appModel.room_ad_a_url;
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:url]];
}
//直播间广告b跳转事件
-(void)openURLb:(UITapGestureRecognizer *)gesture{
    GlobalVariables *fanweApp = [GlobalVariables sharedInstance];
    NSString *url = fanweApp.appModel.room_ad_b_url;
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:url]];
}
- (void)relayoutFrameOfSubViews
{
...
- (void)relayoutOtherContainerViewFrame
{
    ...
    //直播间广告
    CGSize adLabel1Size = [_adLabel1.text boundingRectWithSize:CGSizeMake(MAXFLOAT, kTicketContrainerViewHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:10.5]}  context:nil].size;
    CGSize adLabel2Size = [_adLabel2.text boundingRectWithSize:CGSizeMake(MAXFLOAT, kTicketContrainerViewHeight) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:10.5]}  context:nil].size;
    [_adLabel1 sizeWith:CGSizeMake(adLabel1Size.width, 22)];
    [_adLabel2 sizeWith:CGSizeMake(adLabel2Size.width, 22)];
    [_adLabel1 alignParentLeftWithMargin:kDefaultMargin];
    [_adLabel2 alignParentLeftWithMargin:kDefaultMargin];
    [_adLabel1 layoutParentVerticalCenter];
    [_adLabel2 layoutParentVerticalCenter];
    [_adLabel1 layoutBelow:_otherContainerView margin:kAppMargin2];
    [_adLabel2 layoutBelow:_adLabel1 margin:kAppMargin2];
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section
{
    @synchronized(_userArray)
    {
        ...
        if ([self.is_private isEqualToString:@"1"])//判断是否是私密来返回不同的数据源
        {
            ...
            {
                if (_userArray.count > 0 && _userArray.count <= limitNum)
                {
                    return _userArray.count;
                }
                else if (_userArray.count > limitNum)
                {
                    return limitNum;
                }
                else
