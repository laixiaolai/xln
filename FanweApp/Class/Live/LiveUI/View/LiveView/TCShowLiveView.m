...
- (void)relayoutFrameOfSubViews
{
    ...
    //根据直播间广告的个数来定义头部view的高度
    if(self.fanweApp.appModel.room_ad_a_name.length>0&&self.fanweApp.appModel.room_ad_a_url.length>0){
        [_topView setFrameAndLayout:CGRectMake(0, kStatusBarHeight, rect.size.width, kLogoContainerViewHeight+20+33)];
        if(self.fanweApp.appModel.room_ad_b_name.length>0&&self.fanweApp.appModel.room_ad_b_url.length>0){
            [_topView setFrameAndLayout:CGRectMake(0, kStatusBarHeight, rect.size.width, kLogoContainerViewHeight+20+66)];
        }
    }
    else if (self.fanweApp.appModel.room_ad_b_name.length>0&&self.fanweApp.appModel.room_ad_b_url.length>0){
        [_topView setFrameAndLayout:CGRectMake(0, kStatusBarHeight, rect.size.width, kLogoContainerViewHeight+20+33)];
    }
    else
    {
        [_topView setFrameAndLayout:CGRectMake(0, kStatusBarHeight, rect.size.width, kLogoContainerViewHeight+20)];
    }
...
- (void)sendMessage
{
    ...
    { //普通消息
        ...
        [_liveInputView setText:nil];
//        NSString *userId = [[IMAPlatform sharedInstance].host userId];
//        NSString *userName = [[IMAPlatform sharedInstance].host imUserName];
//        NSString *msg_info = _liveInputView.text;
//        NSMutableDictionary *parmDict = [NSMutableDictionary dictionary];
//        [parmDict setObject:@"message" forKey:@"ctl"];
//        [parmDict setObject:@"chat_record" forKey:@"act"];
//        [parmDict setObject:userId forKey:@"user_id"];
//        [parmDict setObject:[_liveItem liveIMChatRoomId] forKey:@"room_id"];
//        [parmDict setObject:userName forKey:@"user_name"];
//        [parmDict setObject:msg_info forKey:@"msg_info"];
//
//        [_liveInputView setText:nil];
//        [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
//         {
//             NSString *str = [NSString stringWithFormat:@"%@", responseJson[@"status"]];
...
//                 [[FWIMMsgHandler sharedInstance] sendCustomGroupMsg:scmm succ:nil fail:nil];
//             }else{
//                 [FanweMessage alertTWMessage:[NSString stringWithFormat:@"%@", responseJson[@"error"]]];
//             }
//         } FailureBlock:^(NSError *error) {
//             [FanweMessage alertTWMessage:[NSString stringWithFormat:@"%@", error]];
//         }];
    }
}
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    else if([touch.view isKindOfClass:[UIView class]] && (touch.view.tag == kAixinmeiguiTag || touch.view.tag == kChengbaoTag || touch.view.tag == kFalaliTag || touch.view.tag == kHaidiyoulingTag || touch.view.tag == kHuangguanTag || touch.view.tag == kHuojianTag || touch.view.tag == kKongmingTag || touch.view.tag == kNisangTag || touch.view.tag == kQianlichuanqingTag || touch.view.tag == kYoulunTag || touch.view.tag == kFeixuexiaozhenTag || touch.view.tag == kZhishengjiTag || touch.view.tag == kZhushishunliTag || touch.view.tag == kZuanjieTag || touch.view.tag == kTianshiTag || touch.view.tag == kQiubiteTag || touch.view.tag == kXingqiuTag || touch.view.tag == kYingmuhuadaoTag || touch.view.tag == kJitaTag || touch.view.tag == kJiaziguTag || touch.view.tag == kJiebaochaopaoTag))
    {
        return YES;
    }
    else
