...
- (void)showTip{
    if([GlobalVariables sharedInstance].appModel.open_vip_pop){
        NSString *tipMessage = [NSString stringWithFormat:@"开通vip或送礼物升到%ld级即可发言，是否购买vip？",(long)[GlobalVariables sharedInstance].appModel.send_msg_lv];
        [FanweMessage alert:@"发言受限！" message:tipMessage destructiveAction:^{
...
- (void)onClickSend
{
    ...
    if (!_isHost && !isVip && [[IMAPlatform sharedInstance].host getUserRank] < [GlobalVariables sharedInstance].appModel.send_msg_lv)
    {
        //[FanweMessage alertTWMessage:@"您当前等级不能发言！"];
