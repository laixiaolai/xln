...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        [dict setValue:@"app" forKey:@"room_id"];
        ...
        [_httpManager POSTWithParameters:dict SuccessBlock:^(NSDictionary *responseJson) //通过该网络请求来判断功能插件的开启是否冲突
         {
             ...
             {
                 if ([model.class_name isEqualToString:@"live_pay_free"]) {
                     if (_delegate && [_delegate respondsToSelector:@selector(liveFreeClick)])
                     {
                         [self closeClick];
                         [_delegate liveFreeClick];
                     }
                     return ;
                 }
