...
- (TIMCustomElem *)getCustomElemWith:(CustomMessageModel *)customMessageModel
{
    ...
    sender.is_vip=([[GlobalVariables sharedInstance].appModel.open_vip isEqualToString:@"1"] && [[GlobalVariables sharedInstance].is_vip isEqualToString:@"1"])?1:0;
    customMessageModel.sender = sender;
    //加签名
    customMessageModel.sign = [self verificationKey:[NSString stringWithFormat:@"%d",customMessageModel.type]];
    cuserModel * cuser = [[cuserModel alloc]init];
    cuser.is_vip=[[GlobalVariables sharedInstance].appModel.open_vip isEqualToString:@"1"] && [[GlobalVariables sharedInstance].is_vip isEqualToString:@"1"]?1:0;
    customMessageModel.user=cuser;
...
- (void)sendCustomGroupMsg:(SendCustomMsgModel *)sCMM succ:(TIMSucc)succ fail:(TIMFail)fail
{
    if (_chatRoomConversation)
    {
        ...
        customMessageModel.driver_code=sCMM.driver_code;
        customMessageModel.driver_name=sCMM.driver_name;
...
- (void)onRecvC2CSender:(id<IMUserAble>)sender customMsg:(TIMCustomElem *)msg textMsg:(NSString *)textMsg
{
    ...
    if([c2cMsg isKindOfClass:[CustomMessageModel class]]){
        if([GlobalVariables sharedInstance].appModel.open_verification){
            CustomMessageModel *model = (CustomMessageModel *)c2cMsg;
            NSString *key = [self verificationKey:[NSString stringWithFormat:@"%d",model.type]];
            BOOL isVer = [model.sign isEqualToString:key];
            if(model.sign == nil || !isVer){
                return;
            }
        }
    }
...
- (void)onRecvGroupSender:(id<IMUserAble>)sender groupId:(NSString *)groupId customMsg:(TIMCustomElem *)msg textMsg:(NSString *)textMsg
{
    ...
    if ([cachedMsg isKindOfClass:[CustomMessageModel class]]) {
        if ([GlobalVariables sharedInstance].appModel.open_verification) {
            CustomMessageModel* model = (CustomMessageModel*)cachedMsg;
            NSString *key = [self verificationKey:[NSString stringWithFormat:@"%d",model.type]];
            BOOL isVer =[model.sign isEqualToString:key];
            if(model.sign ==nil||!isVer)
            {
...
- (void)onRecvSystepGroupSender:(id<IMUserAble>)sender groupId:(NSString *)groupId customMsg:(TIMGroupSystemElem *)msg textMsg:(NSString *)textMsg
{
    ...
    if ([systepGroup isKindOfClass:[CustomMessageModel class]]) {
        if ([GlobalVariables sharedInstance].appModel.open_verification) {
            CustomMessageModel* model = (CustomMessageModel*)systepGroup;
            NSString *key = [self verificationKey:[NSString stringWithFormat:@"%d",model.type]];
            BOOL isVer =[model.sign isEqualToString:key];
            if(model.sign ==nil||!isVer)
            {
...
- (id<AVIMMsgAble>)getMsgModel:(id<IMUserAble>)sender groupId:(NSString *)groupId customMsg:(NSData *)data textMsg:(NSString *)textMsg
{
    ...
    if (resposeDict && [resposeDict isKindOfClass:[NSDictionary class]])
    {
        ...
        if ([resposeDict count])
        {
            ...
            cuserModel * tmpCuser = [cuserModel mj_objectWithKeyValues:[resposeDict objectForKey:@"user"]];
            customMessageModel.sender = tmpSender;
            customMessageModel.user=tmpCuser;
            customMessageModel.chatGroupID = groupId;
            //如果im消息类型为60，更新后台传过来的脏字库
            if (customMessageModel.type == MSG_FILTER_WORD) {
                GlobalVariables *fanweApp = [GlobalVariables sharedInstance];
                fanweApp.appModel.limit_name=customMessageModel.text;
            }
