...
- (void)createScrollView
{
    ...
    if ([rankModel.host_day isEqualToString:@"1"]) {
        [items addObject:@"日榜"];
    }
    if ([rankModel.host_week isEqualToString:@"1"]) {
        [items addObject:@"月榜"];
    }
    if ([rankModel.host_month isEqualToString:@"1"]) {
        [items addObject:@"总榜"];
    }
    ...
    _cScrollView.contentSize = CGSizeMake(items.count*kScreenW, 0);
