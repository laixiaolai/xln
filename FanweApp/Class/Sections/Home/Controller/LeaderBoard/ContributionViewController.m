...
{
    //分段视图
    AppRankModel *rankModel = [GlobalVariables sharedInstance].appModel.rank_show;
    NSMutableArray *items = [NSMutableArray array];
    if ([rankModel.player_day isEqualToString:@"1"]) {
        [items addObject:@"日榜"];
    }
    if ([rankModel.player_week isEqualToString:@"1"]) {
        [items addObject:@"月榜"];
    }
    if ([rankModel.player_mont isEqualToString:@"1"]) {
        [items addObject:@"总榜"];
    }
    ...
    _bScrollView.contentSize = CGSizeMake(items.count*kScreenW, 0);
