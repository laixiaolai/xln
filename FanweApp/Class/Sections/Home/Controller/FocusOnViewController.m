...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (section == FWFocusOnZeroSection)
    {
        ...
        {
             return ceil(self.dataArr.count/2.0);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == FWFocusOnZeroSection)
    {
        if (self.dataArr.count)
        {
            ...
            if (indexPath.row*2<self.dataArr.count) {
                leftM = self.dataArr[indexPath.row*2];
            }
            if (indexPath.row*2+1<self.dataArr.count) {
                rightM = self.dataArr[indexPath.row*2+1];
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == FWFocusOnZeroSection)
    {
        if (self.dataArr.count)
        {
            ...
            if (indexPath.row*2<self.dataArr.count) {
                tmpModel = [self.dataArr objectAtIndex:indexPath.row*2];
            }
            if (indexPath.row*2+1<self.dataArr.count) {
                rightModel = [self.dataArr objectAtIndex:indexPath.row*2+1];
            }
...
-(void)clickRoomImg:(NSInteger)rowIndex{
    if (rowIndex<self.dataArr.count) {
        HMHotItemModel *tmpModel = [self.dataArr objectAtIndex:rowIndex];
        [self joinOtherLivingRoom:tmpModel];
    }
}
#pragma mark -- 返回分区区头
