...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        return ceil(self.mainDataMArray.count/2.0);
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        {
            HMHotItemModel * leftM;
            HMHotItemModel * rightM;
            if (indexPath.row*2<self.mainDataMArray.count) {
                leftM = self.mainDataMArray[indexPath.row*2];
            }
            if (indexPath.row*2+1<self.mainDataMArray.count) {
                rightM = self.mainDataMArray[indexPath.row*2+1];
            }
            if ([leftM.title isEqualToString:@""]&&[rightM.title isEqualToString:@""] )
            {
                return (kScreenW-10)/2 + 90;
            }
            else{
                return (kScreenW-10)/2 + 110;
            }
        }
        else
        {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.delegate = self;
        HMHotItemModel *tmpModel;
        HMHotItemModel *rightModel;
        if (indexPath.row*2<self.mainDataMArray.count) {
            tmpModel = [self.mainDataMArray objectAtIndex:indexPath.row*2];
        }
        if (indexPath.row*2+1<self.mainDataMArray.count) {
            rightModel = [self.mainDataMArray objectAtIndex:indexPath.row*2+1];
        }
        
        [cell initWidthModel:tmpModel rowIndex:indexPath.row*2 rightModel:rightModel rightRowIndex:indexPath.row*2+1];
        return cell;
    }
}
-(void)clickRoomImg:(NSInteger)rowIndex{
    if (rowIndex<self.mainDataMArray.count) {
