...
@interface HMHomeViewController ()<UIScrollViewDelegate, handleMainDelegate, pushToLiveControllerDelegate, HMHotViewControllerDelegate, SocietyHomePageVCDelegate>
{
    ...
    NSString                      *noticewTitle;         //系统公告标题
    NSString                      *noticewContent;         //系统公告内容
}
...
@property(nonatomic,strong) UIButton * noticeHead;  //系统公告头部
@property(nonatomic,strong) UILabel * noticeTitle;  //系统公告标题
@property(nonatomic,strong) UIScrollView * noticeScrollView;     //系统公告内容
@property(nonatomic,strong) UIWebView * noticeWebView;     //系统公告内容加载
@property(nonatomic,strong) UIButton * noticeClose;     //系统公告内容关闭
...
- (void)initFWUI
{
    ...
    //添加导航背景图片，要对图片进行处理，否则不能自适应
    UIImage *bgImage = [[UIImage imageNamed:@"hm_nav_bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0) resizingMode:UIImageResizingModeStretch];
...
- (void)leftOrRightNavItem
{
...
#else
    // 左上角按钮
    UIView *leftViewContrainer = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 70, kNavigationBarHeight)];
    leftViewContrainer.backgroundColor = kClearColor;
    ...
    [leftViewContrainer addSubview:leftButton];
    //首页导航栏添加客服聊天
    UIButton *chatButton = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetWidth(leftViewContrainer.frame)-55, 0, 93, leftViewContrainer.frame.size.height)];
    ...
    [leftViewContrainer addSubview:chatButton];
    
    UIBarButtonItem *leftButtonItem = [[UIBarButtonItem alloc] initWithCustomView:leftViewContrainer];
...
- (void)updateClassiFiedVC
{
    ...
    @synchronized (self)
    {
        ...
        for (NSInteger i = 0; i < self.classifiedModelMutableArray.count; ++i)
        {
            ...
            UIWebView *webView_bjl = [[UIWebView alloc] initWithFrame:CGRectMake(viewWidth * (i+tmpIndex), 0, viewWidth,self.scrollView.bounds.size.height)];
            [webView_bjl loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:model.url]]];
            webView_bjl.delegate=self;
            
            
            [self.scrollView addSubview:webView_bjl];
...
-(void)goToLiveChat:(UIButton *)chatButton{
    //NSString *str =self.fanweApp.appModel.live_chat_url;
    if(self.fanweApp.appModel.live_chat_url.length>0){
        NSString *str =self.fanweApp.appModel.live_chat_url;
...
//系统公告弹窗
- (void)createsNoticeView
{
    if (_backgroundView == nil) {
        //获取后端
        NSMutableDictionary *parameterDic = [NSMutableDictionary new];
        [parameterDic setValue:@"app" forKey:@"ctl"];
        [parameterDic setValue:@"notice" forKey:@"act"];
        [self.httpsManager POSTWithParameters:parameterDic SuccessBlock:^(NSDictionary *responseJson){
            if([responseJson toInt:@"status"]==1){
                noticewTitle = [responseJson objectForKey:@"title"];
                noticewContent = [responseJson objectForKey:@"content"];
                
                _backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
                _backgroundView.backgroundColor = kAppGrayColor6;
                _backgroundView.alpha = 0.5;
                _backgroundView.hidden = NO;
                [kCurrentWindow addSubview:_backgroundView];
                _bigButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH)];
                [_bigButton addTarget:self action:@selector(closesNoticeView) forControlEvents:UIControlEventTouchUpInside];
                [_backgroundView addSubview:_bigButton];
                _backView = [[UIView alloc] initWithFrame:CGRectMake(10, (kScreenH -300)/2, kScreenW-20, 300)];
                _backView.backgroundColor = [UIColor grayColor];
                _backView.layer.cornerRadius = 5;
                _backView.layer.masksToBounds = YES;
                [kCurrentWindow addSubview:_backView];
                _bigView = [[UIView alloc] initWithFrame:CGRectMake(5, 5, kScreenW-30, 290)];
                _bigView.backgroundColor = [UIColor whiteColor];
                _bigView.layer.cornerRadius = 5;
                _bigView.layer.masksToBounds = YES;
                [_backView addSubview:_bigView];
                
                _noticeHead = [UIButton buttonWithType:UIButtonTypeCustom];
                _noticeHead.frame = CGRectMake(0, 0, kScreenW-30, 40);
                _noticeHead.backgroundColor = kAppMainColor;
                _noticeHead.layer.cornerRadius = 5;
                _noticeHead.layer.masksToBounds = YES;
                [_noticeHead setTitle:@"系统公告" forState:UIControlStateNormal];
                [_bigView addSubview:_noticeHead];
                
                _noticeScrollView =  [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.noticeHead.frame.size.height, _bigView.frame.size.width, 200)];
                _noticeScrollView.contentSize = CGSizeMake(0, 0);
                _noticeScrollView.layer.masksToBounds = YES;
                ...
                [_bigView addSubview:_noticeScrollView];
                _noticeScrollView.contentOffset = CGPointMake(0, 0);
                
                //公告关闭按钮
                _noticeClose = [UIButton buttonWithType:UIButtonTypeCustom];
                _noticeClose.frame = CGRectMake(40, _noticeScrollView.size.height+_noticeHead.size.height+5, _noticeScrollView.size.width-80, 36);
                _noticeClose.backgroundColor = [UIColor grayColor];
                _noticeClose.layer.cornerRadius = 5;
                _noticeClose.layer.masksToBounds = YES;
                [_noticeClose setTitle:@"知道了" forState:UIControlStateNormal];
                [_noticeClose addTarget:self action:@selector(closesNoticeView) forControlEvents:UIControlEventTouchUpInside];
                [_bigView addSubview:_noticeClose];
                
                //加载公告内容
                _noticeWebView = [[UIWebView alloc]initWithFrame:CGRectMake(0, 0, _noticeScrollView.size.width,_noticeScrollView.size.height)];
                _noticeWebView.backgroundColor=[UIColor whiteColor];
                _noticeWebView.delegate=self;
                
                NSURL *url = [[NSBundle mainBundle] URLForResource:@"notice.html" withExtension:nil];
                NSString *htmlString = [NSString stringWithContentsOfURL:url encoding:NSUTF8StringEncoding error:nil];
                //NSString *tempTitle = [htmlString stringByReplacingOccurrencesOfString:@"参数1" withString:noticewTitle];
                
                NSString *bString = noticewContent;
                ...
                NSString *tempContent = [htmlString stringByReplacingOccurrencesOfString:@"参数2" withString:strdeDataBase64];
                
                NSString *basePath = [[NSBundle mainBundle] bundlePath];
                NSURL *baseURL = [NSURL fileURLWithPath:basePath];
                [_noticeScrollView addSubview:_noticeWebView];
                [_noticeWebView loadHTMLString:tempContent baseURL:baseURL];
