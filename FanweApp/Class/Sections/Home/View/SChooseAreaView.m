...
{
    UIButton *sexButton = [[UIButton alloc] initWithFrame: CGRectMake ((kScreenW-100)/2, 10, 100, 30)];
    [sexButton setTitle:@"性别" forState:UIControlStateNormal];
    [sexButton setTitleColor:kAppGrayColor1 forState:UIControlStateNormal];
    sexButton.titleLabel.font=[UIFont systemFontOfSize:14.0];
    [self addSubview:sexButton];
    //[self addSubview:[self creatLabelWithStr:@"性别" andFrame:CGRectMake((kScreenW-100)/2, 0, kScreenW-10, 30)]];
    for (int i = 0; i < 3; i ++)
    {
    ...
    UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(10, kScreenW*0.35, kScreenW-20, 1)];
    ...
    [self addSubview:[self creatLabelWithStr:@"地区" andFrame:CGRectMake((kScreenW-100)/2, kScreenW*0.38, 100, 30)]];
    //NSLog(@"kScreenW:%@",kScreenW);
    self.myTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.bottomBtn.frame)+kScreenW*0.2, kScreenW, self.height-71-kScreenW*0.2-CGRectGetMaxY(self.bottomBtn.frame))];
    ...
    UIView *whiteView = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.myTableView.frame)+10, kScreenW, 60)];
    whiteView.backgroundColor = [UIColor whiteColor];
    [self addSubview:whiteView];
    self.finishBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    self.finishBtn.frame = CGRectMake(10, 10, kScreenW-20, 40);
    
    self.finishBtn.layer.cornerRadius = self.finishBtn.layer.frame.size.height/2;
    self.finishBtn.layer.borderWidth = 1;
    self.finishBtn.layer.borderColor = kAppMainColor.CGColor;
    [self.finishBtn setTitle:@"完成" forState:UIControlStateNormal];
    [self.finishBtn setTitleColor:kAppMainColor forState:UIControlStateNormal];
    ...
    [whiteView addSubview:self.finishBtn];
