...
{
    //底部视图
    self.bottomImgView = [[UIImageView alloc]initWithFrame:CGRectMake(0, self.height -170*kScaleHeight , self.width, 170*kScaleHeight)];
...
- (void)setMyViewWithMArr:(NSMutableArray *)mArr andType:(int)type consumeType:(int)consumeType
{
    if (mArr.count)
    {
        ...
        //是否vip
        if ([model1.is_vip isEqualToString:@"1"])
        {
            self.MVipImgView = [[UIImageView alloc]init];
            self.MVipImgView.frame = CGRectMake(self.MNSRView.width/2-6, (self.MNSRView.height-13)/2-16, 13, 13);
            self.MVipImgView.image = [UIImage imageNamed:@"lr_vip"];
            [self.MNSRView addSubview:self.MVipImgView];
        }
        if (type == 1)
        {
        ...
        if (mArr.count > 1)
        {
            ...
            //是否vip
            if ([model2.is_vip isEqualToString:@"1"])
            {
                self.LVipImgView = [[UIImageView alloc]init];
                self.LVipImgView.frame = CGRectMake(self.LNSRView.width/2-6, (self.LNSRView.height-13)/2-16, 13, 13);
                self.LVipImgView.image = [UIImage imageNamed:@"lr_vip"];
                [self.LNSRView addSubview:self.LVipImgView];
            }
            if (type == 1)
            {
        ...
        if (mArr.count > 2)
        {
            ...
            //是否vip
            if ([model3.is_vip isEqualToString:@"1"])
            {
                self.RVipImgView = [[UIImageView alloc]init];
                self.RVipImgView.frame = CGRectMake(self.RNSRView.width/2-6, (self.RNSRView.height-13)/2-16, 13, 13);
                self.RVipImgView.image = [UIImage imageNamed:@"lr_vip"];
                [self.RNSRView addSubview:self.RVipImgView];
            }
            if (type == 1)
            {
