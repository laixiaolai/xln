...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.startButton.frame = CGRectMake(20, self.height - 120, self.width - 40, (self.width - 40)*0.146);
        ...
        layer.fillColor = kAppMainColor.CGColor;
        layer.strokeColor = kAppMainColor.CGColor;
        ...
        //        [self.startButton setBackgroundImage:[UIImage imageNamed:@"com_bt_nor"] forState:UIControlStateNormal];
        //        [self.startButton setBackgroundImage:[UIImage imageNamed:@"com_bt_sel"] forState:UIControlStateSelected];
        [self addSubview:self.startButton];
        //隐藏封面 开关名字要改
        if([GlobalVariables sharedInstance].appModel.open_verification){
            //封面默认用用户头像
            [self.selectedImageView sd_setImageWithURL:[NSURL URLWithString:[GlobalVariables sharedInstance].head_image] placeholderImage:kDefaultPreloadHeadImg];
            //禁止点击事件
...
- (UIImageView *)backGroundImageview {
    if (_backGroundImageview == nil) {
        ...
        backImage.image =[UIImage imageNamed:@"pl_publishlive_bg"];
