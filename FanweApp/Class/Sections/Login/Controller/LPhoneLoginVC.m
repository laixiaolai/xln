...
- (void)initFWUI
{
    ...
    [self.codeBtn setTitleColor:kAppMainColor forState:UIControlStateNormal];;
    self.codeBtn.layer.cornerRadius = 5;
    self.codeBtn.layer.borderWidth = 1;
    self.codeBtn.layer.borderColor = kAppMainColor.CGColor;
                ...
                [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
                 {
                     ...
                     if ([responseJson toInt:@"status"] == 1)
                     {
                         ...
                         self.fanweApp.appModel.first_login = [responseJson toString:@"first_login"];
                         self.fanweApp.appModel.login_send_score = [responseJson toString:@"login_send_score"];
                         [[GlobalVariables sharedInstance] storageIsVip:[responseJson toString:@"is_vip"]];
                         [[GlobalVariables sharedInstance] storageNick_name:[responseJson toString:@"nick_name"]];
                         [[GlobalVariables sharedInstance] storageHead_image:[responseJson[@"user_info"] toString:@"head_image"]];
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson)
     {
         if ([responseJson toInt:@"status"] == 1)
         {
             ...
             //清除账号缓存
             [[GlobalVariables sharedInstance] storageDriverCode:@""];
             [[GlobalVariables sharedInstance] storageDriverName:@""];
             for (YTMySeatModel* subM in myArr) {
                 if (subM.is_enabled==1) {
                     [[GlobalVariables sharedInstance] storageDriverCode:subM.driver_code];
                     [[GlobalVariables sharedInstance] storageDriverName:subM.driver_name];
                 }
             }
         }else
         {
...
//         if ([responseJson toInt:@"status"] == 1)
//         {
//             userPageModel *userModel = [userPageModel mj_objectWithKeyValues:[responseJson objectForKey:@"user"]];
//             [GlobalVariables sharedInstance].is_vip=userModel.is_vip;
//         }else
//         {
//             [FanweMessage alertHUD:[responseJson toString:@"error"]];
//         }
