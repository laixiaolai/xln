...
- (void)viewDidLoad
{
    ...
    //下拉刷新
    [FWMJRefreshManager refresh:_tableView target:self headerRereshAction:@selector(headerReresh) shouldHeaderBeginRefresh:NO footerRereshAction:nil];
}
...
- (void)doInit
{
    ...
    {
        //self.title = @"VIP充值";
        self.navigationItem.title = @"VIP充值";

    }
    else
    {
        //self.title = @"充值";改用下面的方法，否则底部tab栏会异常
    ...
    //设置tableview高度 top left bottom right
    self.tableView.contentInset=UIEdgeInsetsMake(0, 0, 49+45, 0);
    self.money = @"0";
...
- (void)loadNet
{
    ...
    [self.httpsManager POSTWithParameters:parmDict SuccessBlock:^(NSDictionary *responseJson) {
        ...
        [FWMJRefreshManager endRefresh:_tableView];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 1)
    {
        ...
        if (indexPath.row==self.model.pay_list.count + self.row) {
            [self goToLiveChat:nil];
            return;
        }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    {
        return [self hsOnlyIappay] ? 0 : self.model.pay_list.count + self.row+1;
    }
    else if (section == 2)
    {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (indexPath.section == 1)
    {
        ...
        }else if(indexPath.row==self.model.pay_list.count + self.row){
            
            UITableViewCell* cell = [self.tableView dequeueReusableCellWithIdentifier:@"kefuCell"];
            if (cell==nil) {
                cell=[[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"kefuCell"];
                cell.textLabel.text=@"客服";
                cell.textLabel.font=[UIFont systemFontOfSize:14];
                cell.imageView.image=[UIImage imageNamed:@"com_kefu_gray"];
            }
            return cell;
        }
        else
...
- (UITableViewCell *)payTypeCellWithTbaleview:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        ...
        [imageSelect mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerY.equalTo(cell.contentView);
            make.left.greaterThanOrEqualTo(lbTitles.mas_right).offset(5);
        }];
