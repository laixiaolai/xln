...
{
    [super viewDidLoad];
    UIImage *bgImage = [UIImage imageNamed:@"me_income_bg"];
    self.view.layer.contents = (id) bgImage.CGImage;
    // 如果需要背景透明加上下面这句
    self.view.layer.backgroundColor = [UIColor clearColor].CGColor;
    self.tableView.backgroundColor = [UIColor clearColor];
    
    UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(15,15,kScreenW-30,kScreenH-160)];
    bgView.backgroundColor = RGBA(219, 210, 198, 1);;
    bgView.layer.cornerRadius = 8;
    bgView.layer.masksToBounds = YES;
    [self.view addSubview:bgView];
    UIView *bgView2 = [[UIView alloc] initWithFrame:CGRectMake(3,3,kScreenW-36,kScreenH-166)];
...
- (void)setNavItem
{
    ...
    if (![VersionNum isEqualToString:self.fanweApp.appModel.ios_check_version] )
    {
        ...
        [rightButton setTitleColor:kWhiteColor forState:0];
...
- (void)initHeaderViewWithModel:(BingdingStateModel*)model
{
    ...
    if ([VersionNum isEqualToString:self.fanweApp.appModel.ios_check_version])
    {
        rect = CGRectMake(0, 20, kScreenW, kHeaderViewHight-75-45);
    }
    else
    {
        rect = CGRectMake(0, 20, kScreenW, kHeaderViewHight);
    }
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    section_View.backgroundColor = [UIColor clearColor];
    return section_View;
}
