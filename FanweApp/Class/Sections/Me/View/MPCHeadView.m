...
- (void)creatMyViewWithType:(int)headType
{
    ...
    //self.clearView.backgroundColor = [kAppMainColor colorWithAlphaComponent:0.85];
...
//       self.messageBtn.frame = CGRectMake(kScreenW - 60,20,50,44);
    ...
    @autoreleasepool
    {
        ...
        for (int i = 0; i < array.count; i++)
        {
            ...
            numLabel.textColor    = kAppMainColor;
...
//    self.outLabel.text = [NSString stringWithFormat:@"%@的主页",userId];
    ...
    //之前上头像，现在写死
    [self.clearImgView setImage:[UIImage imageNamed:@"fw_me_head_bg"]];
    //头像
    ...
    if(cModel.is_vip ==1){
        ...
        self.vipImgView.frame = CGRectMake(width+55, (15/2.0f), 15, 15);
        self.vipImgView.image = [UIImage imageNamed:@"lr_vip"];
        [self.nameView addSubview:self.vipImgView];
    }
    //签名
